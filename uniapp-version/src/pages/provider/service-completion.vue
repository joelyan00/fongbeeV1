  ...
  <view class="page-container">
    ...
    <view class="content">
      ...
      <view class="instruction-card">
        <AppIcon name="check-circle" :size="32" style="color: #10b981" />
        <text class="instruction-text">请拍照记录工作完成后的现场照片，并填写服务描述</text>
      </view>

      <!-- Photo Upload -->
      <view class="section">
        <text class="section-title">完工照片（可选）</text>
      ...
      <!-- Description -->
      <view class="section">
        <text class="section-title">工作总结 <text class="required">*</text></text>
        <textarea 
          v-model="description" 
          class="description-input"
          placeholder="请描述工作完成情况及注意事项..."
          :maxlength="500"
        />
...
const handleSubmit = async () => {
  ...
  try {
    const res = await ordersV2Api.submitCompletion(orderId.value, {
      photos: photos.value,
      description: description.value.trim()
    });

    if (res.success) {
      uni.showToast({ title: '验收请求已提交', icon: 'success' });
