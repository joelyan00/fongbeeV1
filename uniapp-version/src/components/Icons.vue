...
const ICONS: Record<string, string> = {
  ...
  truck: '<path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"></path><circle cx="7" cy="18" r="2"></circle><path d="M9 18h4"></path><circle cx="17" cy="18" r="2"></circle><path d="M19 18h2a1 1 0 0 0 1-1v-5l-4-4h-4"></path>',
  ...
  'wechat': '<path d="M11.66 1.63c-3.79 0-6.86 2.87-6.86 6.42 0 1.99.98 3.79 2.52 4.96-.34 1.12-1.25 2.1-1.39 2.22-.16.13.06.36.26.24 1.34-.84 2.84-1.34 3.39-1.46.65.18 1.35.28 2.08.28.32 0 .63-.02.94-.05-.41-.83-.63-1.77-.63-2.75 0-3.69 3.01-6.69 6.72-6.69.41 0 .82.04 1.21.1-1.42-1.92-4.14-3.27-7.24-3.27z"/><path d="M20.25 8.95c-3.14 0-5.69 2.41-5.69 5.39 0 1.68.74 3.16 1.94 4.14-.19.68-1.07 1.83-1.21 2.02-.15.19.1.51.35.35 1.48-.96 2.76-1.57 3.33-1.74.45.1.91.17 1.38.17 3.14 0 5.69-2.41 5.69-5.39s-2.55-5.39-5.69-5.39z"/><circle opacity="0.5" cx="8.58" cy="6.28" r=".63"/><circle opacity="0.5" cx="14.74" cy="6.28" r=".63"/><circle cx="17.17" cy="12.28" r=".53"/><circle cx="23.33" cy="12.28" r=".53"/>',
  ...
  'paw-print': '<circle cx="11" cy="5" r="2"></circle><circle cx="15" cy="9" r="2"></circle><circle cx="7" cy="9" r="2"></circle><circle cx="11" cy="21" r="2"></circle><path d="M11 13c-2.2 0-4 1.8-4 4s1.8 4 4 4 4-1.8 4-4-1.8-4-4-4Z"></path>',
...
export default defineComponent({
  ...
  computed: {
    containerStyle(): Record<string, any> {
      ...
      const normalizeSize = (s: number | string) => {
          if (typeof s === 'number') return `${s}px`;
          if (!isNaN(Number(s))) return `${s}px`;
          return s;
      };

      const sizeValue = normalizeSize(this.size);
      
      return {
        width: sizeValue,
        height: sizeValue,
        minWidth: sizeValue, // Force min-width to prevent collapse
        minHeight: sizeValue, // Force min-height to prevent collapse
        display: 'inline-flex',
        alignItems: 'center',
        justifyContent: 'center',
        lineHeight: '0', // Critical for removing vertical spacing in some contexts
        ...this.style
      };
    },
    svgContent(): string {
    ...
    svgDataUrl(): string {
      ...
      // Ensure strokeWidth is a string/number
      const sWidth = this.strokeWidth;

      const svgStr = `<svg xmlns="http://www.w3.org/2000/svg" width="${this.size}" height="${this.size}" viewBox="0 0 24 24" fill="${fillAttr}" stroke="${strokeAttr}" stroke-width="${sWidth}" stroke-linecap="round" stroke-linejoin="round">${content}</svg>`;
  ...
  mounted() {
    // console.log(`Icon mounted: ${this.name}`);
  }
});
</script>
